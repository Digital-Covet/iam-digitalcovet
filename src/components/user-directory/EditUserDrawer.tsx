...
const EditUserDrawer: Component<EditUserDrawerProps> = (props) => {
  ...
  createEffect(() => {
    ...
    if (user) {
      ...
      setEmail(user.email);
      setShareAccess(user.appAccess.includes("Share"));
      setPortfolioAccess(user.appAccess.includes("Portfolio"));
      setRole(user.role);
      setRequireMfa(user.mfaStatus === "Enabled");
    }
  });
      ...
      <Portal>
        ...
        <Drawer.Positioner class="fixed inset-y-0 right-0 z-70 flex w-full max-w-130">
          <Drawer.Content class="drawer-content flex h-full w-full flex-col bg-card shadow-2xl">
            ...
            {/* Body */}
            <form id="edit-user-form" onSubmit={handleSubmit} class="flex-1 space-y-6 overflow-y-auto px-6 py-6">
              {/* Names */}
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label for="edit-first-name" class="mb-1.5 block text-sm font-medium text-foreground">
                    First Name
                  </label>
                  <input
                    id="edit-first-name"
                ...
                <div>
                  <label for="edit-last-name" class="mb-1.5 block text-sm font-medium text-foreground">
                    Last Name
                  </label>
                  <input
                    id="edit-last-name"
              ...
              {/* Email */}
              <div>
                <label for="edit-email" class="mb-1.5 block text-sm font-medium text-foreground">
                  Work Email Address
                </label>
                <input
                  id="edit-email"
              ...
              {/* Role */}
              <div>
                <label for="edit-role" class="mb-1.5 block text-sm font-medium text-foreground">
                  Assign System Role
                </label>
                <div class="relative">
                  <select
